...
class TestParser:
    ...
    def test_parse_yaml_context_pack(self, tmp_path):
        ...
        yaml_file = synapses_dir / "backend.yaml"
    ...
    def test_document_type_from_path(self, tmp_path):
        ...
        md = concepts_dir / "my-concept.md"
        md.write_text("---\nid: my-concept\n---\n# Concept\nContent here.")
        result = parse_file(str(md))
        assert result["type"] == "concept"

    def test_legacy_systems_path_maps_to_concept(self, tmp_path):
    ...
    def test_design_type_from_path(self, tmp_path):
        designs_dir = tmp_path / "knowledge" / "designs"
        designs_dir.mkdir(parents=True)
        md = designs_dir / "theme.md"
        md.write_text("---\nid: theme-doc\n---\n# Theme\nContent here.")
        result = parse_file(str(md))
